...
async def get_balance_year(year: int = Path(ge=2000, le=2070),
                    db: AsyncSession = Depends(get_db), 
                    current_user_group: fff_schema.UserGroup = Depends(get_current_user_group)):
    return await fff_crud.get_balance_for_year(year, current_user_group, db)

@router.get("/balance/{year}/{month}")
async def get_balance_month(year: int = Path(ge=2000, le=2070),
...
async def get_balance_day(year: int = Path(ge=2000, le=2070),
                    month: int = Path(ge=1, le=12),
...
async def get_summary_year(year: int, 
                           db: AsyncSession = Depends(get_db), 
						   current_user_group: fff_schema.UserGroup = Depends(get_current_user_group)):
    return await fff_crud.get_summary_for_month(year, -1, current_user_group, db)

@router.get("/summary/{year}/{month}")
async def get_summary_month(year: int, month: int, 
                            db: AsyncSession = Depends(get_db), 
							current_user_group: fff_schema.UserGroup = Depends(get_current_user_group)):
    return await fff_crud.get_summary_for_month(year, month, current_user_group, db)