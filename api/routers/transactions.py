...
from api.security import get_current_user, get_current_user_group
...
async def put_transaction(id: int, t_item: fff_schema.TransactionIn, 
						current_user_group: fff_schema.UserGroup = Depends(get_current_user_group), 
						db: AsyncSession = Depends(get_db)):
	t = await fff_crud.get_transaction(id, current_user_group, db)
	if t is None:
...
async def get_transactions_with_ids(tids: str = Query(default="", regex="^[,\d]+$"), 
	...
	return await fff_crud.get_transactions(ids=id_list, current_user_group=current_user_group, db=db)
...
async def put_transactions(t_items: list[fff_schema.TransactionInWithID], 
						current_user_group: fff_schema.UserGroup = Depends(get_current_user_group), 
						db: AsyncSession = Depends(get_db)):
	t_ids = []
